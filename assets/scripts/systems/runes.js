!(function () {
	...
	const e = 'runesData',
		...
		t = 'runeGift',
		o = 'runeUpgrades',
		r = ['water', 'fire', 'earth', 'wizardry'],
		a = { water: '💧', fire: '🔥', earth: '🌍', wizardry: '🔮' };
	let i = { counts: {}, elementals: {}, totalDropped: 0 },
		l = 0,
		s = null,
		c = {
			tripleLuck: !1,
		...
		m = !1;
	function f() {
		return '1' === localStorage.getItem('runesUnlocked');
	}
	function y() {
		(localStorage.setItem(e, JSON.stringify(i)),
			localStorage.setItem(n, String(l)),
			s && localStorage.setItem(t, s),
			localStorage.setItem(o, JSON.stringify(c)));
	}
	function v() {
		return 'undefined' != typeof Beacon ? Beacon.float() : Math.random();
	}
	function g() {
		(p && clearInterval(p),
			'wealth' === s &&
				(p = setInterval(() => {
					'undefined' != typeof points &&
						'function' == typeof updatePointsDisplay &&
						((points += 2e5),
						updatePointsDisplay(),
	...
	function k() {
		u && clearInterval(u);
		const e = Math.round(1e3 / 3);
		u = setInterval(() => {
			const e = document.getElementById('rollBtn');
			!e || e.disabled || 'undefined' == typeof isCutscenePlaying || isCutscenePlaying || e.click();
		}, e);
	}
	function h(e) {
		return 'function' == typeof formatNum ? formatNum(e) : Math.round(e).toLocaleString();
	}
	function w() {
		const e = document.getElementById('runesContainer');
		if (!e) return;
		if (!f())
		...
		e.innerHTML = `\n      <div class="runes-panel">\n        <div style="font-size:1.1em;margin-bottom:4px;">🔷 runes</div>\n        <div style="font-size:0.8em;opacity:0.5;margin-bottom:20px;">runes drop while rolling. collect all 4 elementals for a gift.</div>\n\n        ${s ? `<div class="rune-gift-banner">\n          <span>${'luck' === s ? '⚡' : '💰'} gift of ${s} active</span>\n        </div>` : ''}\n\n        <div class="rune-stat-grid">\n          <div class="rune-stat-card">\n            <div class="rune-stat-label">rare runes</div>\n            <div class="rune-stat-value">${i.counts.rare || 0}</div>\n          </div>\n          <div class="rune-stat-card">\n            <div class="rune-stat-label">mid-rare runes</div>\n            <div class="rune-stat-value">${i.counts['mid-rare'] || 0}</div>\n          </div>\n          <div class="rune-stat-card">\n            <div class="rune-stat-label">common runes</div>\n            <div class="rune-stat-value">${i.counts.common || 0}</div>\n          </div>\n          <div class="rune-stat-card">\n            <div class="rune-stat-label">total dropped</div>\n            <div class="rune-stat-value">${i.totalDropped}</div>\n          </div>\n        </div>\n\n        <div class="rune-section-label">elementals</div>\n        <div class="rune-elemental-grid">\n          ${r
			...
			.map(
				(e) =>
					`\n            <div class="rune-upgrade-item ${c[e.key] ? 'rune-upgrade-owned' : ''}">\n              <div class="rune-upgrade-left">\n                <div class="rune-upgrade-name">${e.emoji} ${e.name}</div>\n                <div class="rune-upgrade-desc">${e.desc}</div>\n              </div>\n              <div class="rune-upgrade-right">\n                ${c[e.key] ? '<span style="font-size:0.8em;color:#4a4;">owned</span>' : `<div style="font-size:0.75em;color:#ffb86b;margin-bottom:4px;">${h(e.cost)} blocks</div>\n                     <button class="small rune-buy-btn" data-key="${e.key}">buy</button>`}\n              </div>\n            </div>\n          `
			)
			.join('')}\n        </div>\n      </div>\n    `;
		const o = e.querySelector('#runeToBlockBtn');
		o &&
			o.addEventListener('click', () => {
				const n = parseInt(e.querySelector('#runeToBlockInput').value) || 0;
				n > 0 &&
					(function (e) {
						if ((i.counts.rare || 0) + (i.counts['mid-rare'] || 0) + (i.counts.common || 0) < e)
							return;
						(i.counts.rare, i.counts['mid-rare'], i.counts.common);
						let n = e;
						for (const e of ['common', 'mid-rare', 'rare']) {
							const t = Math.min(i.counts[e] || 0, n);
							if (((i.counts[e] = (i.counts[e] || 0) - t), (n -= t), n <= 0)) break;
						}
						((l += e * b()), y(), w());
					})(n);
			});
		const d = e.querySelector('#blockToRuneBtn');
		(d &&
			d.addEventListener('click', () => {
				const n = parseInt(e.querySelector('#blockToRuneInput').value) || 0;
				n > 0 &&
					(function (e) {
						const n = Math.ceil(e / 0.85);
						l < n || ((l -= n), (i.counts.common = (i.counts.common || 0) + e), y(), w());
					})(n);
			}),
			e.querySelectorAll('.rune-buy-btn').forEach((e) => {
				e.addEventListener('click', () =>
					(function (e) {
						...
						l < n
							? 'function' == typeof window.showAlert &&
								window.showAlert(`need ${formatNum(n)} blocks!`)
							: ((l -= n),
								(c[e] = !0),
								y(),
								(function (e) {
									('tripleLuck' === e &&
										('function' == typeof recalcLuckMultiplier && recalcLuckMultiplier(),
										...
										'doubleClover' === e &&
											('undefined' != typeof anomalies &&
												((window.anomalies = (window.anomalies || 0) + 5e7),
												'function' == typeof updateAnomalyUI && updateAnomalyUI()),
											'function' == typeof recalcLuckMultiplier && recalcLuckMultiplier(),
											'function' == typeof showAnomalyPopup &&
		...
		(!(function () {
			...
			(function () {
				try {
					...
					i = {
						counts: n.counts || {},
						elementals: n.elementals || {},
						totalDropped: n.totalDropped || 0,
					};
				} catch (e) {}
				((l = parseFloat(localStorage.getItem(n) || '0')), (s = localStorage.getItem(t) || null));
				try {
			...
			document.addEventListener('click', (e) => {
				const n = e.target.closest('.page-dot'),
					t = e.target.closest('#nextPage'),
					o = e.target.closest('#prevPage');
				(n || t || o) && setTimeout(w, 80);
			}));
	}
	((window.getRuneGiftLuckMultiplier = function () {
		return 'luck' === s ? 2 : 1;
	}),
		(window.getRuneTripleLuckMultiplier = function () {
			return c.tripleLuck ? 3 : 1;
		}),
		(window.getRuneDoubleCloverLuckMultiplier = function () {
			return c.doubleClover ? 4 : 1;
		}),
		(window.tryDropRune = function (e) {
			if (!f()) return;
			const n = (function (e) {
					const n = Math.round(1 / e.chance);
					return n >= 1e3 ? 'rare' : n >= 70 ? 'mid-rare' : 'common';
				})(e),
				t = (function (e) {
					return 'rare' === e ? 1 / 30 : 'mid-rare' === e ? 1 / 70 : 1 / 140;
				})(n);
			if (!(v() > t)) {
				if (v() < 0.05) {
					const e = r[Math.floor(v() * r.length)];
					((i.elementals[e] = (i.elementals[e] || 0) + 1),
						i.totalDropped++,
						y(),
						'function' == typeof showAnomalyPopup &&
							showAnomalyPopup(`${a[e]} elemental rune: ${e}!`),
						r.filter((e) => (i.elementals[e] || 0) > 0).length < 4 ||
							s ||
							m ||
							((m = !0),
							(function () {
								const e = document.createElement('div');
								((e.id = 'runeLinkOverlay'),
									(e.style.cssText =
								...
								((n.style.cssText =
									...
									(n.textContent = 'the elements converge...'),
									e.appendChild(n));
								const t = document.createElement('div');
								((t.style.cssText = 'display:flex;gap:32px;font-size:3em;'),
									e.appendChild(t),
														...
														(document.body.removeChild(e),
															...
															(function () {
																...
																const t = (function () {
																	let e = '';
																	for (let n = 0; n < 8; n++)
																		e += 'abcdefghjkmnpqrstuvwxyz23456789'[
																...
																((n.innerHTML =
																	...
																	document.body.appendChild(e));
																let o = null;
																const r = n.querySelector('#giftConfirmArea'),
																	a = n.querySelector('#confirmStrDisplay'),
																	i = n.querySelector('#giftConfirmInput'),
																	l = n.querySelector('#giftConfirmBtn'),
																	c = n.querySelector('#giftCancelBtn');
																(n.querySelectorAll('.gift-opt').forEach((e) => {
																	e.addEventListener('click', () => {
																		((o = e.dataset.gift),
																			n
																				.querySelectorAll('.gift-opt')
																				.forEach((e) => (e.style.opacity = '0.4')),
																			(e.style.opacity = '1'),
																			(e.style.borderColor = 'var(--text-color)'),
																			(a.textContent = t),
																	...
																	i.addEventListener('input', () => {
																		const e = i.value === t;
																		((l.disabled = !e), (l.style.opacity = e ? '1' : '0.5'));
																	}),
																	l.addEventListener('click', () => {
																		o &&
																			...
																			document.body.removeChild(e),
																			'luck' === s &&
																				('function' == typeof recalcLuckMultiplier &&
																					recalcLuckMultiplier(),
																				'function' == typeof showAnomalyPopup &&
																					showAnomalyPopup(
																						'⚡ gift of luck active: 2x permanent luck!'
																					)),
																			'wealth' === s &&
																				(g(),
																				'function' == typeof showAnomalyPopup &&
																					showAnomalyPopup(
																						'💰 gift of wealth active: 200k pts/sec!'
																					)),
																			w());
																	}),
					...
					((i.counts[n] = (i.counts[n] || 0) + 1),
						i.totalDropped++,
						y(),
						'function' == typeof showAnomalyPopup && showAnomalyPopup(`🔷 ${n} rune dropped!`));
				w();
			}
		}),
		(window.renderRunes = w),
