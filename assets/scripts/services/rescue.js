...
(function () {
    ...
    function performResetToDefault() {
        try {
            if (typeof win.flushPlaytime === 'function')
                win.flushPlaytime();
        }
        catch (e) { }
        try {
            if (typeof win.saveAllData === 'function')
                win.saveAllData();
        }
        catch (e) {
    ...
    function onStall(delta) {
        if (level === 0) {
            ...
            level = 1;
            try {
                if (typeof win.forceCleanup === 'function')
                    win.forceCleanup();
                else
                    console.warn('[rescue] forceCleanup unavailable');
            }
            catch (e) {
