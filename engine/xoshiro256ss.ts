const M64 = (1n << 64n) - 1n;

function rotl64(x: bigint, k: bigint): bigint {
  return ((x << k) | (x >> (64n - k))) & M64;
}

function splitmix64(s: bigint): bigint {
  s = (s + 0x9e3779b97f4a7c15n) & M64;
  s = ((s ^ (s >> 30n)) * 0xbf58476d1ce4e5b9n) & M64;
  s = ((s ^ (s >> 27n)) * 0x94d049bb133111ebn) & M64;
  return (s ^ (s >> 31n)) & M64;
}

export class Xoshiro256SS {
  private _s: [bigint, bigint, bigint, bigint] = [0n, 0n, 0n, 0n];

  constructor(seed?: bigint) {
    this.reseed(
      seed !== undefined
        ? seed
        : BigInt(Date.now()) ^
            BigInt(
              Math.floor(
                (typeof performance !== 'undefined' ? performance.now() : 0) * 1e6
              )
            )
    );
  }

  reseed(raw: bigint): void {
    let s = raw & M64;
    for (let i = 0; i < 4; i++) {
      s = splitmix64(s);
      this._s[i] = s;
    }
    if (this._s.every((v) => v === 0n)) this._s[0] = 1n;
  }

  next(): bigint {
    const s = this._s;
    const result = (rotl64((s[1] * 5n) & M64, 7n) * 9n) & M64;
    const t = (s[1] << 17n) & M64;
    s[2] ^= s[0];
    s[3] ^= s[1];
    s[1] ^= s[2];
    s[0] ^= s[3];
    s[2] ^= t;
    s[3] = rotl64(s[3], 45n);
    return result;
  }

  getState(): [string, string, string, string] {
    return this._s.map((v) => v.toString(16).padStart(16, '0')) as [
      string,
      string,
      string,
      string
    ];
  }

  setState(arr: [string, string, string, string]): void {
    this._s = arr.map((v) => BigInt('0x' + v)) as [bigint, bigint, bigint, bigint];
  }

  jump(): void {
    const JUMP: bigint[] = [
      0x180ec6d33cfd0aban,
      0xd5a61266f0c9392cn,
      0xa9582618e03fc9aan,
      0x39abdc4529b1661cn,
    ];
    let s0 = 0n, s1 = 0n, s2 = 0n, s3 = 0n;
    for (let i = 0; i < 4; i++) {
      for (let b = 0; b < 64; b++) {
        if ((JUMP[i] >> BigInt(b)) & 1n) {
          s0 ^= this._s[0];
          s1 ^= this._s[1];
          s2 ^= this._s[2];
          s3 ^= this._s[3];
        }
        this.next();
      }
    }
    this._s[0] = s0 & M64;
    this._s[1] = s1 & M64;
    this._s[2] = s2 & M64;
    this._s[3] = s3 & M64;
  }
}
